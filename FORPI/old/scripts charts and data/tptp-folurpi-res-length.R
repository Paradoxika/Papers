...
pdf('tptp-folurpi-res-length-vs-compress-res-length.pdf', height=5, width=5)
par(mar=c(5.1,4.1,1,2.1))

plot(dataLURPI[,"resOnlyLength"],compressedLengthResOnlyLURPI ,  xlim=c(0,axisLimit), ylim=c(0,axisLimit ),axes=FALSE, xlab="Proof Length (Resolutions)", ylab="Compressed Length (Resolutions)",yaxs="i", xaxs="i")

abline(0,1)
lengths <- sort(unique(dataLURPI[,"resOnlyLength"]))
