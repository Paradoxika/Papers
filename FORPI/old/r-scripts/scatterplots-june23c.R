...
alldataRPI[1:10,]
compressedOnlyDataRPI <- subset(alldataRPI, compressRatioRes > 0, select=proof:numFOSub)
...
alldataLU[1:10,]
compressedOnlyDataLU <- subset(alldataLU, compressRatioRes > 0, select=proof:numFOSub)
...
compressedOnlyDataRPILU <- subset(alldataRPILU, compressRatioRes > 0, select=proof:totalTime)
...
pdf('everything-forpi-folu-length_vs_compress_length_all_proofs.pdf', height=5, width=5)
...
#plot(compressedOnlyDataLURPI[,"length"],LURPIcompressedLenth ,  xlim=c(0,50), ylim=c(0,50),axes=FALSE, xlab="Proof Length", ylab="Compressed Length", main = "Compressed Length vs.\n Proof Length\nIncluding Uncompressed Proofs",yaxs="i", xaxs="i")
plot(compressedOnlyDataLURPI[,"length"],LURPIcompressedLenth ,  xlim=c(0,50), ylim=c(0,50),axes=FALSE, xlab="Proof Length", ylab="Compressed Length",yaxs="i", xaxs="i")
