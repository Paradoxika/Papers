...
\end{scriptsize}
\end{centering}
\caption{The proof for Example \ref{ex:unif}.}
\label{fig:exunif}
\end{figure*}

%straightforward example
\begin{example}\label{ex:unif} 
Consider the proof $\psi$ in Figure \ref{ex:unif}. When computed as in the propositional case, the safe literals for $\eta_3$ are $\{ Q(c), ~ P(a,x)\}$.
...
The node $\eta_3$ in Figure \ref{ex:pairwise} appears to be a candidate for regularization when the safe literals are computed as in the propositional case and unification is considered na\"{i}vely. Note that $\mathcal{S}(\eta_3)=\{Q(c), ~ P(a,x)\}$, and the resolved literal $P(a,c)$ is unifiable with the safe literal $P(a,x)$. 
...
\end{scriptsize}
\end{centering}
\caption{The proof for Example \ref{ex:pairwise}.}
\label{fig:expairwise}
\end{figure*}
...
However, if we attempt to regularize the proof, the same series of actions as in Example \ref{ex:unif} would result in the proof in Figure \ref{fig:expairwise}, which 
requires resolution between $\eta_1$ and $\eta_6$, which is not possible.
...
\end{scriptsize}
\end{centering}
\caption{The proof for Example \ref{ex:unifcheck}.}
\label{fig:exunifcheck}
\end{figure*}
...
In the proof in Figure \ref{fig:exweak1}, note that $\eta_6$ is not strongly regularizable: there is no unifier $\sigma$ such that $\{\lnot P(x),\lnot Q(x),\lnot R(x)\} \sigma \subseteq \mathcal{S}(\eta_6)$.
...
\end{scriptsize}
\end{centering}
\caption{The proof for Example \ref{ex:weak}.}
\label{fig:exweak1}
\end{figure*}
...
Consider the proof of $\psi \setminus \{\eta_7\}$ in Figure \ref{fig:exweak2}.
\begin{figure*}
\begin{centering}
\begin{scriptsize}
\begin{prooftree}
...
\end{scriptsize}
\end{centering}
\caption{The proof for Example \ref{ex:weak} after regularization.}
\label{fig:exweak2}
\end{figure*}
