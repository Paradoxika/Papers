...
returns the intersection, % as shown in Algorithm~\ref{algo:SetSafeLiterals}, 
the latter returns the empty set. 
Moreover, while in {\RPI} the safe literals of the root node contain all the literals of the root clause, in {\RP} the root node's set of safe literals is always empty. % is always assigned an empty set of literals. 