...
Note that  $\ell=\ell'\sigma_1\dots\sigma_k$. By deleting $\eta_2$ from $\psi$, $\eta$ is replaced by $\eta_1$. Thus $\ell^\dagger =  \ell'\sigma_1\dots\sigma_{k-1}$. Thus $\ell^\dagger$ at least as general as $\ell$, which was at least as general as $\ell'$. Since $\ell^\dagger = \ell'$, which is unifiable with $\dual{\ell}'$ by definition, we are done.

%Recall that $\eta$ is strongly regularizable and by assumption, $\eta_1\sigma \subseteq \mathcal{S}(\eta)$. 
%Since $\eta$ is strongly regularizable, we must have that that $\ell\sigma = \ell'\sigma_1\dots\sigma_{k-1}=\ell^\dagger$, so $\ell$ is at least as general as $\ell^\dagger$ which was at least as general as $\ell'$. Thus $\ell^\dagger$ is unifiable with $\dual{\ell}'=\dual{\ell}^\dagger$, as required.
%Since $\eta$ is strongly regularizable, we must have that that 
%$$\ell\sigma = \ell'\sigma_1\dots\sigma_{k-1}$$
%but also by definition of $\ell$ we have that 
%$$\ell=\ell'\sigma_1\dots\sigma_k \implies \ell\sigma = \ell'\sigma_1\dots\sigma_k\sigma$$
%Combing the right hand side of these two equations, we get that
%$$\ell'\sigma_1\dots\sigma_{k-1} = \ell'\sigma_1\dots\sigma_k\sigma$$
%So $\sigma_k\sigma$ must not change any variables in $\ell\sigma$.
