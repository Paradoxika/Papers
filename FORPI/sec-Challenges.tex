...
\noindent
Regularization of the proof by recycling $\eta_1$ results in removing the inference between $\eta_2$ and $\eta_3$, which in turn replaces $\eta_3$ by $\eta_1$. Since $\eta_1$ cannot be resolved against $\eta_4$, and $\eta_1$ contained safe literals, $\eta_5$ is replaced by $\eta_1$. The result is a much shorter proof consisting of a single resolution between $\eta_1$ and $\eta_6$.
%The result is the much shorter proof below.
%\begin{scriptsize}
%\begin{prooftree}
%\def\e{\mbox{\ $\vdash$\ }}
%\AxiomC{$\eta_1$: \e$p(W,X)$}
...
Thus we require the following property be satisfied before regularization.
