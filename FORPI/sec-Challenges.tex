...
Satisfying the pre-regularizability is not sufficient. Consider the proof $\psi$ in Figure \ref{fig:ex-unifcheck}. After collecting the safe literals, $\eta_3$'s safe literals are $\{\lnot q(R,S),\lnot p(c,d), q(f(a,e),c)\}$.
%\noindent
...
Note that there is always at least one node $\eta_\ell$ as used in the definition: the node which resulted in $\ell$ being marked as safe for the path from $\eta$ to the root of the proof. Furthermore, it does not matter which node $\eta_\ell$ is used. To see this, consider some $\eta_\ell' \neq \eta_\ell$ for some $\ell$ with the same pivot. Consider arbitrary nodes $\eta_1$ and $\eta_2$ such that  $\eta_2 \xrightarrow[\sigma_2]{\{\ell\} } \eta_\ell$ and $\eta_1 \xrightarrow[\sigma_1]{\{\ell_1\} } \eta_\ell$; the instantiated resolved literal of $\eta_\ell$ is $\ell\sigma_2$. Now consider arbitrary nodes $\eta_1'$ and $\eta_2'$ such that  $\eta_2' \xrightarrow[\sigma_2']{\{\ell\} } \eta_\ell'$ and $\eta_1' \xrightarrow[\sigma_1']{\{\ell_1'\} } \eta_\ell'$; the instantiated resolved literal of $\eta_\ell'$ is $\ell\sigma_2'$. Since the pivots for $\eta_\ell$ and $\eta_\ell'$ are equal, we must have that $\abs{\ell\sigma_2}=\abs{\ell\sigma_2'}$ and furthermore that $\abs{\ell_1\sigma_1}=\abs{\ell_1'\sigma_1'}$. This shows that it does not matter which $\eta_\ell$ we use; the instantiated resolved literals will always be equal implying that the resolved literals $\ell_1$ and $\ell_1'$ will be contained in both $\mathcal{R}(\eta_\ell)$ and $\mathcal{R}(\eta_\ell')$.
...
In the proof below, note that $\eta_6$ is not satisfy the strongly regularizable: there is no unifier $\sigma$ such that $\sigma \eta_6 \subseteq \mathcal{S}(\eta_6)$ (where $\mathcal{S}(\eta_6) =\{ \lnot p(W), \lnot r(Z), \lnot q(Y)\}$).
\begin{scriptsize}
\begin{prooftree}
\def\e{\mbox{\ $\vdash$\ }}
...
%Note that $\dual{\ell}'=\dual{\ell}^\dagger$ as it corresponds to the opposite literal of $\ell$ in $\psi \setminus \{\eta_2\}$.  
...
Note that  $\ell=\ell'\sigma_1\dots\sigma_k$. By deleting $\eta_2$ from $\psi$, $\eta$ is replaced by $\eta_1$. Thus $\ell^\dagger =  \ell'\sigma_1\dots\sigma_{k-1}$. Recall that $\eta$ is strongly regularizable and by assumption, $\eta_1\sigma \subseteq \mathcal{S}(\eta)$. Since $\eta$ is strongly regularizable, we must have that that $\ell\sigma = \ell'\sigma_1\dots\sigma_{k-1}$, $\ell$ is at least as general as $\ell$ which was at least as general as $\ell'$. Thus $\ell^\dagger$ is unifiable with $\dual{\ell}'=\dual{\ell}^\dagger$, as required.


%Since $\eta$ is strongly regularizable via $\sigma$, we must have that $\ell'\sigma_1\dots\sigma_k\sigma$ in $\psi \setminus\{\eta_2\}$. Thus $\ell^\dagger = \ell'$, which is unifiable with $\dual{\ell}'$ by definition.
