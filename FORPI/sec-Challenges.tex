...
Satisfying the pre-regularizability is not sufficient. Consider the proof $\psi$ in Figure \ref{fig:ex-unifcheck}. After collecting the safe literals, $\eta_3$'s safe literals are $\{\lnot q(T,V),\lnot p(c,d), q(f(a,e),c)\}$.
%\noindent
...
Let $\eta$ be a node satisfying the pre-regularizable, with safe literals $\mathcal{S}(\eta)$ and parents $\eta_1$ and $\eta_2$, assuming without loss of generality that
...
In the proof below, note that $\eta_6$ is not satisfy the strongly regularizable: there is no unifier $\sigma$ such that $\sigma \eta_6 \subseteq \mathcal{S}(\eta_6)$ (where $\mathcal{S}(\eta_6) =\{\lnot q(Y),\lnot r(Z)\}$).
\begin{scriptsize}
\begin{prooftree}
\def\e{\mbox{\ $\vdash$\ }}
...
\end{scriptsize}
\noindent
We show that $\eta_6$ of this proof is weakly regularizable, and that $\eta_7$ can be removed. 
...
Weakly regularizable is implied by the previous two properties, as shown in the next theorem.

\begin{thm}
Let $\eta$ be a node  that is pre-regularizable and strongly regularizable in some proof. Then $\eta$ is weakly regularizable.
\end{thm}

\begin{proof}
Let $\eta$ be a node that satisfies the pre-regularizable and strongly regularizable with parents $\eta_1$ and $\eta_2$.
...
Recall that $\eta$ is strongly regularizable and by assumption, $\sigma\eta_1\subseteq \mathcal{S}(\eta)$. 
