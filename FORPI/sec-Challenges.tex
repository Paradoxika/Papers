...
This property states that a node is pre-regularizable if an instantiated resolved literal $\ell'$ unifiable with a safe literal.
...
The notion of \emph{pre-regulariziability} can be thought of as a \emph{necessary} condition for recycling the node $\eta$, while the notion of \emph{strongly regularizable} can be thought of as a \emph{sufficient} condition. In order to show that this, the notion of subsumption is necessary. We will use $X \sqsubseteq Y$ to denote the following for clauses $X$ and $Y$: there exists a substitution $\sigma$ such that $X\sigma \subseteq Y$. We say that $X$ \emph{subsumes} $Y$.
