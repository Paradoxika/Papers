...
\begin{itemize}
  ...
  \item \textbf{Resolution:} If $\psi_L$ is a proof $\langle V_L, E_L, \clause_L \rangle$ with $\ell_L \in \clause_L$ and
    ...
    \end{align*}
    where $\n$ is a new (resolution) node and $\raiz{\varphi}$ denotes the root node of $\varphi$. The \emph{resolved literal} $\ell$ is such that $\ell = \ell_L \sigma_L = \dual{\ell_R} \sigma_R$ or $\ell = \dual{\ell_L} \sigma_L = \ell_R \sigma_R$.
...
\AxiomC{$\eta_2$} %common
\AxiomC{\hspace{-0.5cm}$\eta_4$: $q(Y)\e p(b)$ } %b
\BinaryInfC{\hspace{-2cm} $\eta_5$: $q(Y) \e$}
\BinaryInfC{$\psi$: $\bot$}
\end{prooftree}
\end{scriptsize}

\noindent
The nodes $\eta_1$, $\eta_2$, and $\eta_4$ are axioms. Node $\eta_3$ is obtained by resolution on $\eta_1$ and $\eta_2$ where $\ell_L =~\vdash p(b)$, $\ell_R = p(b) \vdash$, and $\sigma_L = \sigma_R = \emptyset$. The node $\eta_3'$ is obtained by a contraction on $\eta_3$ with $\sigma=\{X\setminus a\}$. The node $\eta_5$ is the result of resolution on $\eta_2$ and $\eta_4$ with $\ell_L = p(b) \vdash$, $\ell_R = ~ \vdash p(b)$, $\sigma_L=\sigma_R = \emptyset$. Lastly, the conclusion node $\psi$ is the result of a contraction with $\eta_3'$ and $\eta_5$, where $\ell_L = q(a) \vdash$, $\ell_R = q(Y) \vdash$, $\sigma_L = \emptyset$, and $\sigma_R = \{ Y \setminus a\}$. The directed acyclic graph representation of the proof is shown in Figure \ref{fig:dagex}.\\
