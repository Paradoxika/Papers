...
dependent on the capability of the theory reasoners to provide small
explanations.  Decision procedures for uninterpreted symbols (i.e.\ congruence
closure algorithms) date back from the very early days of SMT.  However, to our
best knowledge, the complexity of small explanation generation for set of
literals with uninterpreted symbols (and equalities) has never been properly
studied.  It is believed that the small explanation problem for uninterpreted
symbols is NP-complete.  We show here that this can indeed be proved using a
simple reduction of SAT.
\end{abstract}
