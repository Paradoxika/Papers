...
This is w.l.o.g. since we can always add the axiomatic equations $s=s$ and $t=t$ to $E$, which does not change its congruence closure.
Under this assumption, the set of nodes in the congruence graph are restricted to $\mathcal{T}(E)$. 

There are two kinds of edges in the graph: full edges and congruence
...
Therefore, we characterize complexity results in terms of number of terms and subterms of the input equations, i.e. |T (E)|.
...
$\mathcal{C}= \{C_1, \dots C_n\}$ with variables $x_1,\ldots x_m$.  $\mathcal{I} \models \mathcal{C}$ if and only if
$E_{\mathcal{I}} \cup E_\mathcal{C} \models \hat{c}_1 = \hat{c}'_n$.
Furthermore, $|E_{\mathcal{I}} \cup E_\mathcal{C}|$ is polynomial in $n$ and $m$.
\end{lemma}
\begin{proof}
...
In total, $E_{\mathcal{C}}$ contains at most $n-1 + 2mn$ equations.
\end{proof}
