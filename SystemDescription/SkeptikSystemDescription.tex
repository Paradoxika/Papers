...
Scala's \emph{combinator parsing} library makes it easy to implement parsers for various proof formats. {\skeptik} uses the extension of a file to  determine its proof format. To export proofs in various formats, {\skeptik} provides many classes in exporters package that extend Java's \class{java.io.Writer} class. Available proof formats are:
