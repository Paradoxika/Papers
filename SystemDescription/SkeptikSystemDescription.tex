...
Scala's \emph{combinator parsing} library makes it easy to implement parsers for various proof formats. {\skeptik} uses the extension of a file to  determine its proof format. To export proofs in various formats, {\skeptik} provides many classes in exporters package that extend Java's \class{java.io.Writer} class.
...
The TraceCheck \cite{tracecheck} format, for encoding propositional resolution proofs, represents general proofs as a composition of regular proofs. 
Such proofs can be uniquely reconstructed from its axioms. Therefore auxiliary clauses can be omitted.
Regular proofs are represented by their conclusion clauses, which can be replaced by a '*' if it is not an input clause, and the clauses they were derived from.
The format uses non zero integers to refer both to clauses and literals. Zero is used as a delimiter for the single regular proofs as well as the clause and literal part.\\
This format is used by the PicoSAT solver \cite{Biere_picosatessentials}.
It is one of the three formats accepted at the Certified UNSAT track of the SAT-Competition.\\
TraceCheck proofs can automatically be transformed into RUP proofs \cite{van2008verifying}.\\
Figure \ref{TraceCheckProof} shows a resolution proof in the TraceCheck format and its graphical representation.
\begin{figure}[h]
\makebox[\textwidth][c]{
	...
	\caption{Two representations of a proof}
