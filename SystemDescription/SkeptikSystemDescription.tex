...
%A clause $C_1$ subsumes a clause $C_2$ iff every literal occurring in $C_1$ also occurs in $C_2$. 
...
%
\algo{TopDownSubsumption} (\algo{TDS}) searches for subsumed clauses among all clauses visited earlier in a top-down traversal. 
%
\algo{BottomUpSubsumption} (\algo{BUS}) searches for subsumed clauses among all clauses visited earlier in a bottom-up traversal. A subsumed clause $D$ can only replace a clause $C$, if $D$ is not an ancestor of $C$ in the graph representing the proof. This additional ancestor-check makes it much slower in practice.
%
\algo{RecycleUnits} (\algo{RU}) \cite{RP11} is a special case of \algo{BUS} that only searches for subsuming clauses that are unit (i.e. contain only one literal).
...
%One particularly successful external use of {\skeptik} was in interpolation-based controller synthesis: a proof with ToDo nodes was compressed by ToDo\% and better interpolants could be extracted from the compressed proof \cite{ToDo}.
