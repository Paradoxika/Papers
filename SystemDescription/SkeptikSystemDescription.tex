...
Although current automated deduction tools are very efficient at finding proofs, they do not necessarily find the best proofs. {\skeptik} (\url{http://github.com/Paradoxika/Skeptik/}) aims precisely at efficiently analyzing proofs, in order to compress and improve them by eliminating their redundancies.
...
Scala's combinator parsing library makes it easy to implement parsers for various proof formats. {\skeptik} uses the extension of a file to  determine its proof format. To export proofs in various formats, {\skeptik} provides many classes in exporters that extend Java's \class{java.io.Writer} class.
