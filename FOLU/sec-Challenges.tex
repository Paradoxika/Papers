...
Even if $\ell$ satisfied the last property, these unifiers $\sigma_i$ might not be applied in the case of postponing resolution with $u$ (or more generally, because another unit clause has been postponed). The following proof provides a concrete example, where resolution with $u=\eta_2$ is to be postponed.
\begin{footnotesize}
\begin{prooftree}
\def\e{\mbox{\ $\vdash$\ }}
