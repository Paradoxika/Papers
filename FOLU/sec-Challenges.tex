...
%example 1: shows requirement for pair-wise unifiability with unit
 \begin{example}\label{ex:unif} Resolution with a unit clause $u$, with literal $\ell$, may be performed with a node $v$ provided $\dual{\ell} \in \Gamma_v$ and there exists a unifier $\sigma$ of $\ell$ and $\dual{\ell}$. After applying $\sigma$ to the premises, the literals match syntactically, and this behaves like the propositional case. Thus the notion of looking for unifiable formulas to postpone resolution with $u$ is natural. Consider the following proof of $\psi$, where resolution with $\eta_2$ will be postponed:
...
After postponing resolution with $\eta_2$, the formulas $p(Y) \in \Gamma_{\eta_1}$ and $p(X)\in \Gamma_{\eta_4}$ will both remain as in $\eta_3'$ below. Unlike the propositional case, where repeated literals would be dropped, in order to compress the proof, we must first contract these literals into a single literal, e.g. $p(X) \in \Gamma_{\eta_4'}$. Reintroduction of $u$ completes the proof via resolution with the contracted literal.
\begin{footnotesize}
\begin{prooftree}
\def\e{\mbox{\ $\vdash$\ }}
\AxiomC{$\eta_1'$: $p(Y)$\e$q(Z)$}
...
Let $u$ be a unit clause with literal $\ell$, and let $\dual{\ell}_1 \in \Gamma_{v_1},\ldots,\dual{\ell}_n \in \Gamma_{v_n}$ be the literals that are unified with $\ell$ during resolution between $v_i$ and $u$ in a proof $\psi$. 
%Then there should be a unifier $\sigma$ such that for every $\dual{\ell}_i$, there exists a unifier of $\dual{\ell}_i$ and $\ell\sigma$.
Then there should be a unifier $\sigma$ of  $\{\dual{\ell}_1,\ldots,\dual{\ell}_n,\ell\}$.
\end{property}
...
%Then there should be a unifier $\sigma$ such that for every $\dual{\ell}_i^r$, there exists a unifier of $\dual{\ell}_i^r$ and $\ell\sigma$.
Then there should be a unifier $\sigma$ of $\{\dual{\ell}_1^r, \ldots, \dual{\ell}_n^r, \ell\}$.
\end{property}
