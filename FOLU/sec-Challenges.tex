...
Let $u$ be a unit clause with literal $\ell$, and let $\dual{\ell}_1 \in \Gamma_{v_1},\ldots,\dual{\ell}_n \in \Gamma_{v_n}$ be the literals that are unified with $\ell$ during resolution between $v_i$ and $u$ in a proof $\psi$. Then there should be a unifier $\sigma$ such that for every $\dual{\ell}_i$, there exists a unifier of $\dual{\ell}_i$ and $\ell\sigma$.
\end{property}
