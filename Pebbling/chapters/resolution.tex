...
Consider the unsatisfiable propositional logic formula $\Phi$ in conjunctive normal form.
...
\ap(v,f) = \Big\{
\begin{array}{ll}
	f(v,\ap(pr_1,f),\ap(pr_2,f)) &\text{ if } v \text{ has premises } pr_1 \text{ and } pr_2\\
	g_f(v) &\text{ otherwise}\\
\end{array}
$$
...
f(v,w_1,w_2) = \left\{
\begin{array}{ll}
	\top & \text{ if $v$ has no premises} \\
	w_1 \wedge w_2 &\text{ if the conclusion of $v$ is a resolvent}\\
