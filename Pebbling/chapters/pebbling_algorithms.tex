...
\begin{algorithm}[h]
  ...
  \While{$N$ is not empty}{
    choose $v \in N$ heuristically\;
		$S = S ::: (v)$; \tcp*[f]{$:::$ is the concatenation of sequences}\\
		$N = N \setminus \{v\}$\;
		\For(\tcp*[f]{check whether $c$ is now pebbleable}){\KwSty{each} $c \in \Children{v}{\varphi}$}{ 
...
\begin{algorithm}[h]
  ...
  \KwOut{sequence of nodes}
	...
	$D = D \cup \{v\}$; \\
	$N = \Premises{v}{\varphi} \setminus D$; \tcp*[f]{Only unprocessed premises are visited} \\
	$S_1 = S$\;
	
  \While{$N$ is not empty}{
    choose $p \in N$ heuristically;
		$N = N \setminus p$\;
		$S_1 = S_1 ::: BUpebble(\varphi,p,D,S)$; \tcp*[f]{$:::$ is the concatenation of sequences}
  }
