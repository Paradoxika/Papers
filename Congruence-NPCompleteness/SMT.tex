...
We use the usual notions of assignments, model, (un)satisfiability also for
...
function symbol of arity $n$ in $\mathcal{F}$ and $t_1,\ldots,t_n$ are terms in $\mathcal{T}^{\Sigma}$.
\end{definition}
...
%
An \emph{equation} $s = t$ on terms in a set $\mathcal{T}$ can also be see as a singleton relation $(s,t) \in \mathcal{T} \times \mathcal{T}$.  By extension, a set of equation can also be seen as a relation, that is, the union of the singleton relations.  A quantifier-free first-order logic literal is a propositional variable, an equation or the negation of any of the first two.
...
The congruence closure $E^*$ of a set of equations $E$ on a set of terms closed under the subterm relation $\mathcal{T}$ is the smallest congruence relation containing $E$.
...
Such an assignment is a model for a formula $F$, if 
\begin{itemize}
