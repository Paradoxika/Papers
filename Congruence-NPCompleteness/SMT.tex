...
This raised the question whether we could construct shortest conflict clauses as shortest paths in such weighted congruence graphs, using a polynomial time shortest path algorithm.
...
The reason why the shortest path method is not able to find shortest conflict clauses is that the weights for congruence edges can not be accurately determined in a priori.
...
From compatability and the fact that $\hat{x}_j~=~\top_j~\in~E_{\mathcal{I}}$ it follows that $E \cup E_{\mathcal{I}} \models t_i(\hat{x}_j)~=~t_i(\top_j)$.
