...
with the same top symbol and labeled by the compatibility-deduced equality
...
merges two classes, either because of an input equality or because an
compatibility-deduced equality, the graph is added respectively a full or
congruent edge.  Because an edge is only added between two classes while
merging, the path between two terms in the same class is unique.  The
...
equalities.  There is not even a guarantee that the explanation is minimal.
...
would only be recorded in the congruence graph if it is considered before $a =
b$.
...
minimal.  The next section proves it is deceptive to look for low complexity
techniques to provide the smallest conflict.
